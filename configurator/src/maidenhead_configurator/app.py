...
from .firmware import FirmwareUpdater, find_rp2040_volumes
from .models import (
    DATE_FORMATS,
    GNSS_MODES,
    ConfigError,
...
from .serial_transport import NdjsonSerialTransport, SerialDevice, discover_devices
...
class ConfiguratorApp(tk.Tk):
    ...
    def __init__(self, client: Client | None = None) -> None:
        ...
        self._known_devices: dict[str, SerialDevice] = {}
        self._connected_device: SerialDevice | None = None
    ...
    def _build(self) -> None:
        ...
        self.device_combo = ttk.Combobox(
            header, textvariable=self.connection_choice, state="readonly", width=44
        )
    ...
    def _refresh_usb(self) -> None:
        try:
            ...
            self._known_devices = {device.label: device for device in devices}
    ...
    def _connect(self) -> None:
        ...
        transport: NdjsonSerialTransport | None = None
        try:
            if choice == "Simulator":
                client = Client(SimulatedTransport())
                selected_device = None
            else:
                selected_device = self._known_devices.get(choice)
                if selected_device is None:
                    raise ProtocolError(
                        "device_selection_stale",
                        "refresh USB and explicitly select the target device",
                    )
                port = selected_device.port
    ...
    def _build_device(self, frame: ttk.Frame) -> None:
        self.device_info = tk.StringVar(
            value="Select Refresh USB then Connect to use a physical device."
        )
        ttk.Label(
            frame, textvariable=self.device_info, justify="left", font=("TkFixedFont", 10)
        ).pack(anchor="w")
        ttk.Button(
            frame, text="Refresh device information", command=self._refresh_device_info
    ...
    def _refresh_device_info(self) -> None:
        try:
            ...
            self.device_info.set(
                json.dumps({"info": info, "diagnostics": diagnostics}, indent=2, sort_keys=True)
            )
        except ProtocolError as exc:
    ...
    def _build_display_builder(self, frame: ttk.Frame) -> None:
        ttk.Label(
            frame, text="Bottom row blocks (maximum 16 characters at their widest format)"
        ...
        ttk.Combobox(
            editor,
            textvariable=self.block_kind,
            state="readonly",
            values=("battery", "time", "date", "text", "space", "separator"),
            width=14,
        ).grid(row=1, column=0, sticky="ew")
        ttk.Label(editor, text="Text / separator value").grid(
            row=2, column=0, sticky="w", pady=(8, 0)
        )
        ttk.Entry(editor, textvariable=self.block_value, width=18).grid(
            row=3, column=0, sticky="ew"
        )
        ttk.Button(editor, text="Update block", command=self._update_block).grid(
    ...
    def _move_block(self, direction: int) -> None:
        ...
        if index is None or not 0 <= index + direction < len(self.config_model.bottom_blocks):
            return
        self._render_blocks(
            reorder_blocks(self.config_model.bottom_blocks, index, index + direction)
        )
    ...
    def _update_block(self) -> None:
        ...
        if index is None:
            return
        self.config_model.bottom_blocks[index] = DisplayBlock(
            self.block_kind.get(), self.block_value.get()
        )
        self._render_blocks(index)

    def _build_behavior(self, frame: ttk.Frame) -> None:
        self.behavior_vars = {
            "gnss_mode": tk.StringVar(),
            "tracking_interval_seconds": tk.StringVar(),
            "acquisition_timeout_seconds": tk.StringVar(),
            "dim_deadline_seconds": tk.StringVar(),
            "shutdown_deadline_seconds": tk.StringVar(),
            "normal_brightness_percent": tk.StringVar(),
            "dim_brightness_percent": tk.StringVar(),
        }
        ttk.Label(frame, text="GNSS behavior").grid(row=0, column=0, sticky="w")
        ttk.Combobox(
            frame,
            textvariable=self.behavior_vars["gnss_mode"],
            state="readonly",
            values=sorted(GNSS_MODES),
        ).grid(row=0, column=1, sticky="ew")
        rows = (
            ("tracking_interval_seconds", "Tracking redraw interval (seconds)"),
            ("acquisition_timeout_seconds", "GPS acquisition timeout (seconds)"),
            ("dim_deadline_seconds", "Dim after LOCATE edge (seconds)"),
            ("shutdown_deadline_seconds", "Shutdown after LOCATE edge (seconds)"),
            ("normal_brightness_percent", "Normal brightness (%)"),
            ("dim_brightness_percent", "Dim brightness (%)"),
        )
        for row, (field, label) in enumerate(rows, 1):
            ttk.Label(frame, text=label).grid(row=row, column=0, sticky="w", pady=4)
            if "brightness" in field:
                ttk.Scale(
                    frame,
                    from_=0,
                    to=100,
                    variable=self.behavior_vars[field],
                    command=lambda value, variable=self.behavior_vars[field]: variable.set(
                        str(round(float(value)))
                    ),
                ).grid(row=row, column=1, sticky="ew", padx=10)
                ttk.Label(frame, textvariable=self.behavior_vars[field], width=4).grid(
                    row=row, column=2, sticky="w"
                )
                continue
            maximum = 60 if field == "tracking_interval_seconds" else 600
            ttk.Spinbox(
                frame, from_=0, to=maximum, textvariable=self.behavior_vars[field], width=12
            ).grid(row=row, column=1, sticky="w", padx=10)
        ttk.Label(
            frame,
            text="All timers are measured from the initial LOCATE button-down edge.",
            wraplength=560,
        ).grid(row=8, column=0, columnspan=2, sticky="w", pady=12)
    ...
    def _build_time(self, frame: ttk.Frame) -> None:
        ...
        self.timezone_combo = ttk.Combobox(
            frame, textvariable=self.timezone_var, values=sorted(available_timezones()), width=38
        )
        self.timezone_combo.grid(row=0, column=1, sticky="ew", padx=8)
        ttk.Checkbutton(
            frame, text="24-hour clock", variable=self.clock_24h_var, command=self._refresh_preview
        ).grid(row=1, column=0, columnspan=2, sticky="w", pady=8)
        ttk.Checkbutton(
            frame, text="Show seconds", variable=self.seconds_var, command=self._refresh_preview
        ).grid(row=2, column=0, columnspan=2, sticky="w")
        ttk.Label(frame, text="Date format").grid(row=3, column=0, sticky="w", pady=8)
        ttk.Combobox(
            frame, textvariable=self.date_format_var, state="readonly", values=sorted(DATE_FORMATS)
        ).grid(row=3, column=1, sticky="w", padx=8)
        ttk.Label(
            frame,
            text="The device receives generated offset transitions; time zone is never inferred from coordinates.",
            wraplength=580,
        ).grid(row=4, column=0, columnspan=2, sticky="w", pady=12)
        frame.columnconfigure(1, weight=1)

    def _build_profiles(self, frame: ttk.Frame) -> None:
        ttk.Label(
            frame, text="Profiles contain settings only—never coordinates or live diagnostics."
        ).pack(anchor="w")
        buttons = ttk.Frame(frame)
        buttons.pack(anchor="w", pady=12)
        ttk.Button(buttons, text="Save current settings…", command=self._save_profile).pack(
            side="left"
        )
        ttk.Button(buttons, text="Open and compare…", command=self._open_profile).pack(
            side="left", padx=8
        )
        ...
        if path:
            notes = (
                simpledialog.askstring(
                    "Profile notes", "Optional notes for this profile:", parent=self
                )
                or ""
            )
    ...
    def _open_profile(self) -> None:
        ...
        if messagebox.askyesno(
            "Apply profile",
            "Apply this profile to the editor? It will not be written until Validate and apply.",
        ):
    ...
    def _build_firmware(self, frame: ttk.Frame) -> None:
        ...
        ttk.Entry(frame, textvariable=self.uf2_path_var, width=56).grid(
            row=1, column=0, sticky="ew", pady=4
        )
        ttk.Button(frame, text="Browse…", command=self._browse_uf2).grid(row=1, column=1, padx=8)
        ttk.Button(frame, text="Validate UF2", command=self._validate_uf2).grid(
            row=2, column=0, sticky="w", pady=8
        )
        ttk.Button(frame, text="Back up profile and install…", command=self._install_firmware).grid(
            row=3, column=0, sticky="w"
        )
        ttk.Button(
            frame, text="Install using BOOTSEL recovery…", command=self._install_recovery
        ).grid(row=4, column=0, sticky="w", pady=(8, 0))
        ttk.Label(
            frame,
            text="Recovery: unplug the device, hold its BOOTSEL control, plug USB back in, then select the RPI-RP2 drive if automatic detection cannot find it.",
            wraplength=610,
    ...
    def _validate_uf2(self) -> None:
        try:
            ...
            self.status.set(
                f"Valid RP2040 UF2: {info.block_count} blocks, {info.payload_bytes} payload bytes"
            )
    ...
    def _install_firmware(self) -> None:
        ...
        try:
            ...
            if self._connected_device is None:
                raise ProtocolError(
                    "identity_not_provided", "connect to the physical target before updating"
                )
            backup_path = updater.backup_profile(backup)
            existing_volumes = updater.snapshot_boot_volumes()
            updater.enter_bootloader()
            if self._serial_transport:
                self._serial_transport.close()
                self._serial_transport = None
            try:
                volume = updater.wait_for_boot_volume(exclude=existing_volumes)
            except ProtocolError as error:
                if error.code != "boot_volume_not_found":
                    raise
                selected = filedialog.askdirectory(
                    title="Select the mounted RPI-RP2 bootloader drive"
                )
                if not selected:
            ...
            device = updater.wait_for_reconnect(
                boot_volume=volume, expected_device=self._connected_device
            )
            self._known_devices[device.label] = device
            self.connection_choice.set(device.label)
            self._connect()
            info = self.client.request("get_info")
            version = (
                info.get("firmware_version", "unknown") if isinstance(info, dict) else "unknown"
            )
            self.status.set(
                f"Firmware {version} verified; profile backup saved as {backup_path.name}"
            )
        except ProtocolError as exc:
            self._show_error("Firmware update", exc)

    def _install_recovery(self) -> None:
        source = self.uf2_path_var.get()
        try:
            info = inspect_uf2(source)
        except (OSError, ValueError) as exc:
        ...
        if not messagebox.askyesno(
            "BOOTSEL recovery",
            f"Install {Path(source).name} ({info.block_count} blocks) through ROM recovery? "
            "The application firmware is unavailable, so settings cannot be backed up now.",
        ):
            return
        try:
            volumes = find_rp2040_volumes()
            if len(volumes) > 1:
                raise ProtocolError(
            ...
            if volumes:
                ...
                selected = filedialog.askdirectory(title="Select the mounted RPI-RP2 drive")
                if not selected:
                    return
                volume = Path(selected)
            updater = FirmwareUpdater(self.client)
            preexisting_devices = updater.snapshot_devices()
            updater.copy_uf2(source, volume)
            self.status.set("Recovery firmware copied. Waiting for USB reconnect…")
            device = updater.wait_for_reconnect(
                boot_volume=volume, preexisting_devices=preexisting_devices
            )
            self._known_devices[device.label] = device
            self.connection_choice.set(device.label)
            self._connect()
            ...
            self.status.set(f"Recovery complete; firmware {version} verified")
        except ProtocolError as exc:
            self._show_error("Firmware recovery", exc)

    def _build_factory_reset(self, frame: ttk.Frame) -> None:
        ttk.Label(
            frame,
            text="This restores defaults on the device. It does not erase firmware and does not retain coordinates.",
            wraplength=580,
        ).pack(anchor="w")
        ttk.Button(frame, text="Restore factory settings…", command=self._factory_reset).pack(
            anchor="w", pady=16
        )
    ...
    def _candidate_from_widgets(self) -> DeviceConfig:
        ...
        if hasattr(self, "behavior_vars"):
            ...
            data.update(
                ...
                clock_24h=self.clock_24h_var.get(),
    ...
    def _factory_reset(self) -> None:
        if not messagebox.askyesno(
            "Factory reset", "Restore factory settings on the connected device?"
        ):
            return
        try:
